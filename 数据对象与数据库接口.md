...
String pid;             // 用户身份证ID，需要保证长度为18
String phone;           // 手机号，需要保证长度为11
Sex sex;                // 性别枚举：M,F
```
...
```java
int serialNumber;           // 序列号，非空
String apid;                // 订单ID，需要保证长度为12
String aid;                 // 与其关联的用户的AID
String did;                 // 与其关联的医生的DID
int sid;                    // 与其关联的排班的SID
AppointmentStatus status;   // 状态枚举
LocalDateTime startTime;    // 开始时间（同SID对应的排班的时间）
LocalDateTime endTime;      // 结束时间（同SID对应的排班的时间）
```
