...
+ 使用maven拉取依赖：项目根目录运行命令`./mvnw clean package`
...
可以把源代码拷贝两份分别运行客户端和服务端，也可以在`server`目录和`client`目录分别使用`./mvnw clean package`将客户端和服务端分别打包为jar，使用jar运行。（运行后会有类似于
```bash
[INFO] --- jar:3.4.1:jar (default-jar) @ habms-server ---
[INFO] Building jar: E:\Xprogram\java\HABMS\server\target\habms-server-0.1.0-SNAPSHOT.jar
```
的提示指示jar包在哪里）
