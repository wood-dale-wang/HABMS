...
返回：data对象：医生列表，其中每个元素都包含基本数据（DID,Name,Department,Description）
可能的错误类型：未登录（通过服务器缓存判断）、科室不存在、DID不存在
注：若提供Name，则返回所有同名医生列表。
...
发送：data对象：DID,SID
返回：data对象：Appointment对象（包含doctorName, department）
...
发送：data对象：无
返回：data对象：Appointment对象列表（包含doctorName, department）
可能的错误类型：未登录（通过服务器缓存判断）
...
发送：data对象：修改后的Schedule对象（可修改DID, STime, ETime, Capacity）
返回：data对象：更新后的Schedule对象
可能的错误类型：未登录（通过服务器缓存判断）、SID不存在、不是Admin、容量小于已预约数
...
发送：data对象：DoctorAccount对象列表（包含passwordHex数据，不包含DID）
返回：data对象：DoctorAccount对象列表（包含DID，不包含passwordHex数据）
可能的错误类型：未登录（通过服务器缓存判断）、不是Admin
...
发送：data对象：DID
返回：data对象：无
可能的错误类型：未登录、不是Admin、DID不存在

### 删除排班（Admin）

发送：data对象：SID
返回：data对象：无
可能的错误类型：未登录、不是Admin、SID不存在
...
- startTime: 开始时间
- endTime: 结束时间
- status: 状态（Ok:已预约, Abandon:已取消, Done:已完成）
